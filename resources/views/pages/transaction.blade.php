...
@section('content')
	...
	<form method="post" x-data="{{ json_encode($data) }}">
		...
		<fieldset>
			...
			<label><x-input name="kind" type="radio" value="owing" :checked="$transaction->kind == 'owing'" /> Someone owes money</label>
			<label><x-input name="kind" type="radio" value="payment" :checked="$transaction->kind == 'payment'" /> Someone payed money</label>
		...
		<fieldset x-cloak
			...
			<legend x-show="kind == 'drivetrak' && car_id && my_cars.includes(parseInt(car_id)) && other_user_id && distance">When
		...
		<fieldset x-cloak
			...
			<x-input name="memo" type="text" :value="$transaction->memo" placeholder="It's optional" />
		</fieldset>
		...
		<span x-cloak
			...
			x-transition>Aprox amount: <span
				x-text="axios.get(`/calculate/trip-price?car_id=${car_id}&distance=${distance}&date=${occurred_at}`).then(r => r.data).catch(e => e.code)"></span></span>
